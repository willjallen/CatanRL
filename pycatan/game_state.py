...
# Game:
# assigned player num
assigned_player_num = np.array([int(i == )])

# curent player playing [0, 3]
current_player_playing = np.array([int(i == self.game.player_with_turn_index) for i in range(10)])

# Trade:
# player's trading partner [0,2] (OHE)
trading_player = np.array([int(i == 5) for i in range(10)])
# Offered resource cards [0, 4] (OHE)
offered_resource_cards = np.array([int(i == 5) for i in range(10)])
# Expected resource cards [0, 4] (OHE)
expected_resource_cards = np.array([int(i == 5) for i in range(10)])

# Player:
# turn to play [0,1] (OHE)
turn_to_play = np.array([int(i == 5) for i in range(10)])
# pending trade [0,1] (OHE)
pending_trade = np.array([int(i == 5) for i in range(10)])
# available actions [0, 14] (OHE)
available_actions = np.array([int(i == self.) for i in range(14)])
